...
colors = ['xkcd:purple', 'xkcd:sky blue', 'xkcd:orange']
...
df_final = df_final[df_final['vaccine_coverage_1st_dose']==0.5]
for s, channel in enumerate(['Infections', 'Severe', 'Mortality']):
    ...
    for x, (group_plot, df_gp) in enumerate(df_final.groupby(['scenario', 'target_group'])):
            ...
            if 'Infections' in channel:
                ...
                axs[k, s].set_yticks([0, 0.05, 0.1, 0.15, 0.2])
                axs[k, s].set_yticklabels(['0', '', '10%', '', '20%'])
            elif 'Severe' in channel:
                axs[k, s].set_ylim([0, 0.4])
                axs[k, s].set_yticks([0, 0.1, 0.2, 0.3, 0.4])
                axs[k, s].set_yticklabels(['0', '', '20%', '', '40%'])
            elif 'Mortality' in channel:
...
plt.suptitle('Prioritizing distribution by age')
plt.subplots_adjust(left=0.08, bottom=0.2, right=0.88)
fig_name = os.path.join(fig_dir, 'covax_age_priority.png')
